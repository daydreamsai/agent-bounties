...
const CHAINS: Record<string, { client: PublicClient; rpcUrl: string }> = {
  ethereum: { client: createPublicClient({ chain: mainnet, transport: http("https://eth.llamarpc.com") }) as PublicClient, rpcUrl: "https://eth.llamarpc.com" },
  arbitrum: { client: createPublicClient({ chain: arbitrum, transport: http("https://arb1.arbitrum.io/rpc") }) as PublicClient, rpcUrl: "https://arb1.arbitrum.io/rpc" },
  optimism: { client: createPublicClient({ chain: optimism, transport: http("https://mainnet.optimism.io") }) as PublicClient, rpcUrl: "https://mainnet.optimism.io" },
  polygon: { client: createPublicClient({ chain: polygon, transport: http("https://polygon-rpc.com") }) as PublicClient, rpcUrl: "https://polygon-rpc.com" },
  base: { client: createPublicClient({ chain: base, transport: http("https://mainnet.base.org") }) as PublicClient, rpcUrl: "https://mainnet.base.org" },
};
 ...
 */
function calcIL(currentPriceRatio: number): number {
  if (currentPriceRatio <= 0) return 0;
 ...
 */
function calcFeeAPR(totalVolumeUSD: number, tvlUSD: number, windowHours: number): number {
  if (tvlUSD <= 0 || windowHours <= 0) return 0;
  const feeRate = 0.003;
...
async function getPoolInfo(chain: string, poolAddress: string): Promise<PoolInfo> {
  ...
  const [token0Addr, token1Addr] = await Promise.all([
    chainConfig.client.readContract({ address: addr, abi: PAIR_ABI, functionName: "token0" } as any) as Promise<`0x${string}`>,
    chainConfig.client.readContract({ address: addr, abi: PAIR_ABI, functionName: "token1" } as any) as Promise<`0x${string}`>,
  ]);
  ...
  const [dec0, dec1] = await Promise.all([
    chainConfig.client.readContract({ address: token0Addr, abi: ERC20_ABI, functionName: "decimals" } as any) as Promise<number>,
    chainConfig.client.readContract({ address: token1Addr, abi: ERC20_ABI, functionName: "decimals" } as any) as Promise<number>,
  ]);

  let sym0 = "UNKNOWN", sym1 = "UNKNOWN";
  try {
    [sym0, sym1] = await Promise.all([
      chainConfig.client.readContract({ address: token0Addr, abi: ERC20_ABI, functionName: "symbol" } as any) as Promise<string>,
      chainConfig.client.readContract({ address: token1Addr, abi: ERC20_ABI, functionName: "symbol" } as any) as Promise<string>,
    ]);
...
async function getHistoricalPrices(
  ...
  token1Symbol: string,
  hours: number
): Promise<{ priceRatio: number; volumeUSD: number }> {
  try {
    ...
    if (!data.prices || data.prices.length < 2) throw new Error("No price data");
    ...
    try {
      ...
      if (r1.ok) {
        const d1 = await r1.json() as Record<string, { usd: number }>;
