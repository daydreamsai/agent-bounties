...
function paymentsFromEnv(configOverrides) {
  const baseConfig = {
    ...
    facilitatorAuth: configOverrides?.facilitatorAuth ?? process.env.FACILITOR_AUTH ?? process.env.FACILITATOR_AUTH ?? process.env.PAYMENTS_FACILITATOR_AUTH ?? process.env.DREAMS_AUTH_TOKEN,
...
function createFacilitatorAuthHeaders(token) {
...
function encodePaymentRequiredHeader(details) {
  const payload = {
    x402Version: 2,
    ...details
  };
...
function extractRecipientAddress(_request, response) {
  const required = decodePaymentRequiredHeader(
    response.headers.get("PAYMENT-REQUIRED")
  );
...
async function createRuntimePaymentContext(options) {
  ...
  if (options.privateKey) {
    ...
    const caip2Network2 = networkToCaip2(options.network);
    if (!caip2Network2) {
      logWarning(
        options.logger,
    ...
    try {
      ...
      const signer2 = toClientEvmSigner(account);
      const client = new x402Client();
      client.register(caip2Network2, new ExactEvmScheme(signer2));
      const fetchWithPayment = attachPreconnect(
        wrapFetchWithPayment(baseFetch, client),
        baseFetch
      );
      const chainId2 = inferChainId(options.network);
      return {
        fetchWithPayment,
        signer: signer2,
        walletAddress: account.address,
        chainId: chainId2 ?? null
      };
    } catch (error) {
      logWarning(
        options.logger,
  ...
  const signer = new Proxy(signerTarget, {
    get(target, prop) {
      if (prop === "address") {
        return runtimeSigner.account.address ?? ZERO_ADDRESS;
      }
      return Reflect.get(target, prop);
    }
  });
  const caip2Network = options.network ? networkToCaip2(options.network) : `eip155:${chainId}`;
  if (!caip2Network) {
    logWarning(
      options.logger,
  ...
  try {
    ...
    const client = new x402Client();
    client.register(caip2Network, new ExactEvmScheme(signer));
    const fetchWithPayment = attachPreconnect(
      wrapFetchWithPayment(fetchWithPolicy, client),
      baseFetch
    );
    return {
      fetchWithPayment,
...
var NETWORK_CONFIG, toStringMessage, normalizeAddressOrNull;
...
// src/pricing.ts
function validatePriceFormat(price, entrypointKey) {
  ...
  if (price && typeof price === "object") {
    ...
    if ("amount" in priceObj) {
      console.warn(
        `[lucid-agents/payments] Invalid price format for entrypoint "${entrypointKey}": { amount: ${priceObj.amount} } is not valid. Use string format: price: "${priceObj.amount}" or object format: { invoke: "${priceObj.amount}" }`
      );
      return false;
    }
    ...
    const hasValidValues = Object.values(priceObj).every(
      (val) => val === void 0 || typeof val === "string"
    );
    if (!hasValidKeys || !hasValidValues) {
      console.warn(
        `[lucid-agents/payments] Invalid price format for entrypoint "${entrypointKey}": Expected { invoke?: string; stream?: string } but got ${JSON.stringify(price)}`
      );
      return false;
    }
    return true;
  ...
  if (typeof price === "number") {
    console.warn(
      `[lucid-agents/payments] Invalid price format for entrypoint "${entrypointKey}": Price must be a string, not a number. Use: price: "${price}"`
    );
    return false;
  }
  return false;
...
function entrypointHasExplicitPrice(entrypoint) {
  ...
  if (typeof price === "object") {
    if ("amount" in price) {
      const priceObj = price;
      console.warn(
        `[lucid-agents/payments] Invalid price format for entrypoint "${entrypoint.key}": { amount: ${priceObj.amount} } is not valid. Use string format: price: "${priceObj.amount}" or object format: { invoke: "${priceObj.amount}" }`
      );
      return false;
    }
  ...
  if (typeof price === "number") {
    console.warn(
      `[lucid-agents/payments] Invalid price format for entrypoint "${entrypoint.key}": Price must be a string, not a number. Use: price: "${price}"`
    );
  }
  return false;
}
...
var createX402Fetch = ({
  ...
  for (const network of networksToRegister) {
    ...
    if (caip2Id) {
      client.register(caip2Id, new ExactEvmScheme(signer));
    }
  }
  console.info(
...
export { ZERO_ADDRESS, accountFromPrivateKey, createAgentCardWithPayments, createFacilitatorAuthHeaders, createInMemoryPaymentStorage, createPaymentTracker, createPaymentsRuntime, createPostgresPaymentStorage, createRateLimiter, createRuntimePaymentContext, createSQLitePaymentStorage, createX402Fetch, createX402LLM, decodePaymentRequiredHeader, encodePaymentRequiredHeader, entrypointHasExplicitPrice, evaluateIncomingLimits, evaluateIncomingPolicyGroups, evaluateOutgoingLimits, evaluatePaymentRequirement, evaluatePolicyGroups, evaluateRateLimit, evaluateRecipient, evaluateSender, extractPayerAddress, extractSenderDomain, findMostSpecificIncomingLimit, findMostSpecificOutgoingLimit, normalizeAddress, parsePriceAmount, paymentRequiredResponse, payments, paymentsFromEnv, resolveActivePayments, resolvePaymentRequirement, resolvePrice, sanitizeAddress, validatePaymentsConfig, wrapBaseFetchWithPolicy, x402LLM };
